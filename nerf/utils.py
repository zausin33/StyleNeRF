...
from .RAIN import Net as RAIN_net
...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None, patch_size=1, subsampling:str=None):
    ''' get rays
    Args:
    ...
    if N > 0:
        ...
        if patch_size > 1:
            if subsampling == "random":
                ...
                select_inds = torch.tensor(select_inds).to(device)
                inds = select_inds.expand([B, N])
            elif subsampling == "downsample":
                total_inds = np.arange(H * W).reshape(H, W)
                patch_H, patch_W = patch_size, patch_size
                num_region_H, num_region_W = (H-1)//patch_H, (W-1)//patch_W

                region_position_v = np.random.randint(H - patch_H*num_region_H)
                region_position_u = np.random.randint(W - patch_W*num_region_W)
                # select_inds = total_inds[region_position_v:][:patch_H:2][:, region_position_u:][:, :patch_W:2].reshape(-1)
                select_inds = total_inds[region_position_v:][:patch_H*num_region_H:num_region_H][:, region_position_u:][:, :patch_W*num_region_W:num_region_W].reshape(-1)
                select_inds = torch.tensor(select_inds).to(device)
                inds = select_inds.expand([B, N])
...
class Trainer(object):
    def __init__(self, 
        ...
        if optimizer is None:
            self.optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=0.001, weight_decay=5e-4) # naive adam
        else:
        ...
        if self.workspace is not None:
            if self.opt.stage == "second":
                self.load_checkpoint(from_first_stage=True, model_only=True)
                for name, param in self.model.named_parameters():
                    learn_layer = False
                    for layer_to_learn in self.opt.learn_layer:
                        if name.startswith(layer_to_learn):
                            learn_layer = True
                    if not learn_layer:
        ...
        self.nnfm_loss_fn = NNFMLoss(device=self.device)
        self.net = RAIN_net(self.opt, self.device)

        self.log(f'[INFO] Trainer: {self.name} | {self.time_stamp} | {self.device} | {"fp16" if self.fp16 else "fp32"} | {self.workspace}')
        self.log(f'[INFO] #parameters: {sum([p.numel() for p in model.parameters() if p.requires_grad])}')
        self.log(f'[INFO] #layers: {[name for name, param in model.named_parameters() if param.requires_grad]}')
    ...
    def train_step(self, data):
        ...
        if 'images' not in data:
            raise Exception("no images in data")

        images = data['images']  # [B, N, 3/4]

        render_whole_image = self.opt.stage == "second" and self.opt.subsampling == "deferred"
        ...
        if self.opt.stage == "second" and self.opt.subsampling == "deferred":
            with torch.no_grad():
        ...
        if self.opt.stage == "second":
            if self.opt.style_loss == "nnfm":
                if self.opt.subsampling == "deferred":
                    gt_rgb = gt_rgb.permute(0, 3, 1, 2).contiguous()
                    pred_rgb = pred_rgb.permute(0, 3, 1, 2).contiguous()
                else:
                    gt_rgb = gt_rgb.transpose(2, 1).reshape(1, 3, self.opt.patch_size, self.opt.patch_size)
                    pred_rgb = pred_rgb.transpose(2, 1).reshape(1, 3, self.opt.patch_size, self.opt.patch_size)
                pred_rgb.requires_grad_(True)

                loss_dict = self.nnfm_loss_fn(
                    pred_rgb,
                    self.style_image.permute(2, 0, 1).unsqueeze(0),
                    blocks=[
                        self.opt.vgg_block,
                    ],
                ...
                loss_dict["content_loss"] *= self.opt.content_weight
                ...
                content_feat = self.net.get_content_feat(gt_rgb)
                output_content_feat = self.net.get_content_feat(pred_rgb)

                output_style_feats, output_style_feat_mean_std = self.net.get_style_feat(pred_rgb)

                style_feats, style_feat_mean_std = self.net.get_style_feat(self.style_image.cuda().unsqueeze(0))

                content_loss = get_content_loss(content_feat, output_content_feat, content_weight=self.opt.content_weight)
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        # save final result
        self.save_checkpoint(full=True, best=False, final=True)
    ...
    def train_gui(self, train_loader, step=16):
        ...
        for _ in range(step):
            ...
            if self.opt.stage == "second" and self.opt.subsampling == "deferred":
                ...
                imageio.imwrite(
                    os.path.join(self.workspace, 'checkpoints', f'logim_{self.global_step}.png'),
                    np.clip(preds.cpu().detach().squeeze(0).permute(1, 2, 0).numpy() * 255.0, 0.0, 255.0).astype(np.uint8),
                )

                rgb_pred_grad = preds.grad.permute(0, 2, 3, 1).contiguous().clone().detach().view(1, -1, 3)
                ...
                rays_o = data['rays_o'] # [B, N, 3]
                rays_d = data['rays_d'] # [B, N, 3]
                patch_size = 5000  # or some other patch size

                # Next, we back-propagate the cached gradients to scene parameters and accumulate in a patch-wise manner;
                # for each patch, we re-render it with enabled auto-differentiation, and apply the chain rule to back-propagate the
                # corresponding cached patch gradients to scene parameters for accumulation

                start = random.randint(0, H*W - patch_size)
                end = start + patch_size

                #for view_batch_start in range(0, H * W, patch_size):
                patch_rgb_pred_grad = rgb_pred_grad[:,  start : end] # view_batch_start : view_batch_start + patch_size]
                outputs = self.model.render(
                    rays_o[:, start : end],  #view_batch_start : view_batch_start + patch_size],
                    rays_d[:, start : end],#view_batch_start : view_batch_start + patch_size],
                    staged=False,
                    bg_color=1,
                    perturb=True,
                    force_all_rays=True,
                ...
                patch_pred_rgb = outputs['image']
                patch_pred_rgb.backward(patch_rgb_pred_grad)
                #    rgb_pred.append(patch_pred_rgb.clone().detach())
                #rgb_pred = torch.cat(rgb_pred, dim=1).reshape(H, W, 3)
    ...
    def test_gui(self, pose, intrinsics, W, H, bg_color=None, spp=1, downscale=1):
        ...
        rays = get_rays(pose, intrinsics, rH, rW, -1, subsampling=self.opt.subsampling)
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            if self.opt.stage == "second" and self.opt.subsampling == "deferred":
                ...
                for view_batch_start in range(0, H * W, patch_size):
                    patch_rgb_pred_grad = rgb_pred_grad[:, view_batch_start : view_batch_start + patch_size]
                    outputs = self.model.render(
                        rays_o[:, view_batch_start : view_batch_start + patch_size],
                        rays_d[:, view_batch_start : view_batch_start + patch_size],
                        staged=False,
                        bg_color=1,
                        perturb=True,
                        force_all_rays=True,
                    ...
                    patch_pred_rgb = outputs['image']
                    patch_pred_rgb.backward(patch_rgb_pred_grad,) #retain_graph=True)
                    rgb_pred.append(patch_pred_rgb.clone().detach())
                rgb_pred = torch.cat(rgb_pred, dim=1).reshape(H, W, 3)
    ...
    def save_checkpoint(self, name=None, full=False, best=False, remove_old=True, final=False):
        ...
        if name is None:
            name = f'{self.name}_stage:{stage}_ep{self.epoch:04d}'
    ...
    def load_checkpoint(self, checkpoint=None, model_only=False, from_first_stage=False):
        if checkpoint is None:
            ...
            name = f"{self.name}_stage:{stage}_ep*"
            checkpoint_list = sorted(glob.glob(f'{self.ckpt_path}/{name}.pth'))
            if checkpoint_list:
...
def get_style_input(opt, device):
    if (opt.stage == "second" and opt.style_loss == "nnfm") or opt.color_transfer:
        ###### resize style image such that its long side matches the long side of content images
        style_img = imageio.imread(opt.style_img_path).astype(np.float32) / 255.0
        style_h, style_w = style_img.shape[:2]
        content_long_side = max([opt.W, opt.H])
        if style_h > style_w:
        ...
        imageio.imwrite(
            os.path.join(opt.workspace, 'checkpoints', 'style_image.png'),
            np.clip(style_img * 255.0, 0.0, 255.0).astype(np.uint8),
        )
        style_img = torch.from_numpy(style_img).to(device=device)
        print("Style image: ", opt.style_img_path, style_img.shape)
    else:
        ori_style_img = Image.open(opt.style_img_path).convert('RGB')
